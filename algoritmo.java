...
public class GA{
  ...
  private static final int geracao = 200;
  private final double mutacao = 0.05;
  privata static final double crossover = 0.9;
  ...
  public ordenar(){
    ...
    public void Avaliador() {
    	...
    	for( i = 0; i < populacao; i++){
    		...
    		//organiza as soluções de acordo com score
    		this.organizarSolucoes();
      ...
      public void Mutacao(int aux5) {
        ...
        double valor2 = Math.round(num); //gerando valor aleatório com valor obtido anteriormente
        
        int OutroNum = (int)valor2;
        
        int valorTroca = valor.nextInt(n_releases + 1);

        proxGeracao[axu5][OutroNum] = valorTroca;
    ...
    public void crossoverFunct(){
      ...
      totalScore = this.scoreFinal();
    	...
    	this.roulette(totalScore);
    	
    	for(i = 0; i<populacao; i = i+2) {	
      
    		if(Math.random() <= TAXACRUZAMENTO) {
          
          //gerando vetores auxiliares
      ...
      public void Exibir() {
    	...
    	for(i = 0; i < geracao; i++) {
    		...
    		System.out.println(" " + this.scoreMedio());
    		this.crossoverFunct();
